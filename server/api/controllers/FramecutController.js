...
var IScaler = {
  basePath:appPath + "/uploads/framecut/temp/"
  ...
  , mergeImgs:function(mergeConfig,callback){
    ...
    async.times( filesCount ,function(n,next){
      ...
      async.times( filesCount ,function(n,next){
        ...
        .write(targetDir+"framecut-merged.png", function (err) {
          if (err) {
            console.log('append error:');
            console.log(err);
          }
          execFile(pngquant, ['--quality=50-50', targetDir+"framecut-merged.png"], function (err) {
              if (err) {
                console.log("png8a error:");
                console.log(err);
              }
              var tempFile = targetDir + "framecut-merged-fs8.png"
              var optedFile = optDir + "framecut-merged.png"
      ...
      req.file('file').upload(function (err, files) {
        ...
        if(files[0].filename.indexOf(".zip")!=-1){
          ...
          async.each([config],IScaler.mergeImgs,function(){
            IScaler.copyfile(dir+'/zip/merge/framecut-merged.png',dir+'/package/soruce.png',function(){
              IScaler.copyfile(dir+'/zip/optmiezd/framecut-merged.png',dir+'/package/optmiezd.png',function(){
                //生成下载文件
                setTimeout(function(){
